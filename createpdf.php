...
if (isset($_GET['ReportRange'])) {
    ...
    elseif ($_GET['ReportRange'] == "custom") {
        ...
        if (isset($_GET['startdate'])) {
            if ($_GET['startdate'] == "") {
                echo "<span style=\"color: red; \"><h1>"._("Startdate is missing!")."</h1></span> \n";
                echo _("When setting custom report period, startdate is required")."\n";
                exit;
            }
        }
        ...
        if ($starttime > $endtime) {
            echo sprintf("<span style=\"color: red; \"><h1>%s</h1></span>\n", _("Startdate need to be before tomorrow or end date!"));
            exit;
        } elseif ($endtime - $starttime < 3600) {
            echo sprintf("<span style=\"color: red; \"><h1>%s</h1></span>\n", _("Time frame need to be minimum 1 hour!"));
            exit;
        }
    }
    else {
        echo sprintf("%s\n", _("Unknown time report range!"));
        exit;
    }
}
...
if ($reporttype == 'host') {
    if (!is_numeric($hostid)) { echo _("ERROR: Need hostid for host report!"); exit; }
...
elseif ($reporttype == 'hostgroup') {
    if (!is_numeric($groupid)) { echo _("ERROR: Need groupid for group report!"); exit; }
    ...
    echo _("Report type not selected!")."\n";
    exit;
}
...
if (!file_exists($tmp_pdf_data)) {
    echo _("Report ") . $tmp_pdf_data . _(" not found! Cannot continue to create PDF.");
    exit;
}
...
$pdf->addText(50,34,6,_("Generated by Zabbix Monitoring Dynamic Report v") . $version);
$pdf->restoreState();
$pdf->closeObject();
